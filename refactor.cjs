...
content = content.replace(
  ...
  `<ThemeSection id="overview" className="bg-gradient-to-br from-emerald-50 via-teal-50/30 to-cyan-50" noBg>`
);
...
// Replace specific elements in Categories Section
content = content.replace(
  ...
  `<ThemeCard className="group p-8 hover:-translate-y-2 relative overflow-hidden border-slate-200/60 hover:border-blue-300/50">`
);
content = content.replace(
  ...
  `<ThemeCard className="group p-8 hover:-translate-y-2 relative overflow-hidden border-slate-200/60 hover:border-rose-300/50">`
);

// Close ThemeCards properly in Categories
content = content.replace(/<\/div>\s*<\/Reveal>\s*<Reveal delayMs=\{15\}>/g, `</ThemeCard>\n              </Reveal>\n              <Reveal delayMs={15}>`);
content = content.replace(/<\/div>\s*<\/Reveal>\s*<\/div>\s*\{?\/\* <Reveal delayMs=\{50\}>/g, `</ThemeCard>\n              </Reveal>\n            </div>\n            {/* <Reveal delayMs={50}>`);
...
// Adjust closing tags of the resources mapped cards
content = content.replace(/<\/div>\s*<\/Reveal>\s*\)\)/g, `</ThemeCard>\n                </Reveal>\n              ))`);

// Let's replace Buttons / links inside the cards to use ThemeButton
content = content.replace(
  /<Link\s*to=\{resource.referenceUrl\}\s*className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 text-white px-4 py-2 rounded-lg font-semibold hover:from-emerald-600 hover:to-teal-600 transition-all duration-300 text-sm inline-flex items-center justify-center after:absolute after:inset-0"\s*>/g,
  `<ThemeButton to={resource.referenceUrl} className="w-full text-sm inline-flex items-center justify-center after:absolute after:inset-0 rounded-none z-20">`
);
content = content.replace(
  /<Link\s*to=\{resource.referenceUrl\}\s*className="w-full bg-gradient-to-r from-emerald-500 to-teal-500 text-white px-3 py-2 rounded-lg font-semibold hover:from-emerald-600 hover:to-teal-600 transition-all duration-300 text-xs inline-flex items-center justify-center after:absolute after:inset-0"\s*>/g,
  `<ThemeButton to={resource.referenceUrl} className="w-full text-xs inline-flex items-center justify-center after:absolute after:inset-0 rounded-none z-20">`
);

content = content.replace(/Read Article\s*<\/Link>/g, `Read Article\n                        </ThemeButton>`);
