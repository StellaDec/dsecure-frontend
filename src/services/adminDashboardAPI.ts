...
interface DashboardStats {
  ...
  availableLicenses: string | number;
...
export class AdminDashboardAPI {
  // Get dashboard statistics — uses /api/Performance/managed-stats
  // PURANA CODE: return apiCall<DashboardStats>('/admin/dashboard/stats') — 404 tha
  // NAYA CODE: Working endpoint + JWT-only auth (X-User-Email header causes 500 on this endpoint)
  static async getDashboardStats(): Promise<ApiResponse<DashboardStats>> {
    try {
      ...
      // Check if data is missing or has encrypted flag still present (decryption failed silently)
      if (!data || data.encrypted === true) {
        return {
          success: false,
          data: null as unknown as DashboardStats,
          error: "Failed to decrypt dashboard stats response",
        };
      }

      // Transform managed-stats response to DashboardStats format
      const stats: DashboardStats = {
        totalLicenses: data.totalLicenses ?? 0,
        activeUsers: data.activeUsers ?? 0,
        availableLicenses: Math.max(
          0,
          (data.totalLicenses ?? 0) - (data.licensesInUse ?? 0),
        ),
        successRate: "100%",
        totalUsers: data.totalUsers ?? 0,
        licensesInUse: data.licensesInUse ?? 0,
        totalReports: data.totalReports ?? 0,
        lastUpdated: data.lastUpdated,
        changes: {
          totalLicenses: { value: "", trend: "up" },
          activeUsers: { value: "", trend: "up" },
          availableLicenses: { value: "", trend: "up" },
          successRate: { value: "", trend: "up" },
        },
      };
  ...
  static async getUserActivity(
    ...
    try {
      ...
      const transformedData: UserActivity[] = response.data.subusers.map(
        (subuser) => ({
          email: subuser.email,
  ...
  static async assignBulkLicenses(
    userCount: number,
    licenseCount: number,
  ...
  static async restartMachine(hostname: string): Promise<ApiResponse<any>> {
    return apiCall<any>(`/admin/machines/${hostname}/restart`, {
      method: "POST",
    });
  }

  static async runErase(hostname: string): Promise<ApiResponse<any>> {
    return apiCall<any>(`/admin/machines/${hostname}/erase`, {
      method: "POST",
    });
  }
  ...
  static async regenerateReport(reportId: string): Promise<ApiResponse<any>> {
    return apiCall<any>(`/admin/reports/${reportId}/regenerate`, {
      method: "POST",
    });
  }
  ...
  static async resetUserPassword(userId: string): Promise<ApiResponse<any>> {
    return apiCall<any>(`/admin/subusers/${userId}/reset-password`, {
      method: "POST",
    });
  }

  static async toggleUserStatus(userId: string): Promise<ApiResponse<User>> {
    return apiCall<User>(`/admin/subusers/${userId}/toggle-status`, {
      method: "POST",
    });
  }
  ...
  static async assignLicensesToGroup(
    groupId: string,
    licenseData: {
      licenseCount: number;
      expiryDate: string;
  ...
  // Erasure Metrics API
  static async getErasureMetrics(
    data: ErasureMetricsRequest,
  ): Promise<ApiResponse<ErasureMetricsResponse>> {
    const response = await apiCall<ErasureMetricsResponse>(
      "/api/Performance/erasure-metrics",
      {
        method: "POST",
        body: JSON.stringify(data),
      },
