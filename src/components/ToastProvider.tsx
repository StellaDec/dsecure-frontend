...
export const ToastProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const getToastStyles = (type: string) => {
    ...
    switch (type) {
      case 'success':
        return `${baseStyles} bg-[#d4ede4] border border-[#d4ede4] text-[#0a2e1e]`;
      case 'error':
  ...
  const getIcon = (type: string) => {
    switch (type) {
      case 'success':
        return (
          <svg className="w-5 h-5 text-[#0a2e1e]" fill="currentColor" viewBox="0 0 20 20">
    ...
    <>
      ...
      {toast && (
        <div className={getToastStyles(toast.type)}>
          <div className="flex items-start p-4 rounded-none shadow-lg">
