...
export default function AdminLicenses() {
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const dataToExport =
        filteredLicenses.length > 0 ? filteredLicenses : licenseList;
  ...
  const handleExportPDF = async () => {
    ...
    try {
      ...
      try {
        ...
        doc.setTextColor(30, 41, 59);
        doc.text(
          `Total: ${licenses.total}  |  Active: ${licenses.active}  |  Inactive: ${licenses.inactive}  |  Expired: ${licenses.expired}`,
          14,
          40,
        );
        ...
        autoTable(doc, {
          head: [
            ["#", "License Key", "User Email", "Type", "Status", "Expires"],
          ],
          body: tableData,
          startY: 50,
  ...
  const uniqueLicenseTypes = Array.from(
    new Set(licenseList.map((l) => l.license_type || "Unknown")),
  ).sort();
  ...
  const confirmRevoke = async () => {
    ...
    try {
      ...
      const targets = licenseList.filter((l) =>
        revokeData.ids.includes(l.license_id),
      );
                  ...
                  <PieChart>
                    ...
                    >
                      {licenseDetails.map((entry, index) => {
                        ...
                        const fillColor =
                          entry.color && colorMap[entry.color]
