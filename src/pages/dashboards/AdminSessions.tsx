...
export default function AdminSessions() {
  ...
  useEffect(() => {
    ...
    const currentSession = sessions.find(
      (s) =>
        s.user_email === currentUserEmail &&
        s.isActive &&
        s.estimatedExpiryTime,
    );
    ...
    const groupedActivities = activityLog.reduce(
      (acc, item) => {
        ...
        if (existing) {
          ...
          acc.push({ type: item.type, time: item.time, count: 1 });
        }
        return acc;
      },
      [] as { type: string; time: string; count: number }[],
    );
      ...
      <div className="flex flex-col gap-1.5">
        {groupedActivities.map((item, idx) => (
          <div key={idx} className="flex items-center gap-2 text-xs">
            ...
            >
              {item.type?.replace("_", " ")}
              {item.count > 1 && <span className="ml-1">({item.count})</span>}
            </span>
            ...
            <div className="divide-y divide-slate-100">
              ...
              sessions.length === 0 ? (
                ...
                paginatedData.map((session, idx) => (
                  <div
                    key={idx}
                    className="grid grid-cols-1 md:grid-cols-12 gap-4 p-4 hover:bg-slate-50 transition-colors items-start"
                  >
                    {/* 1. User & Status */}
                    <div className="col-span-12 md:col-span-2">
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-1.5">
                          <span
                            className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase border w-fit ${getStatusColor(session.session_status)}`}
                          >
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="font-medium text-slate-600 text-xs">
                          {session.lastActiveTime
                            ? formatDate(session.lastActiveTime)
                            : "-"}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col gap-2 text-xs">
                        ...
                        <div className="border-t border-slate-200 pt-2">
                          ...
                          <span
                            ...
                            title={String(session.device_info)}
                          >
                            {typeof session.device_info === "object"
